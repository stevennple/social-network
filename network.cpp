...
Network::Network(std::vector<User*> users) {
    users_ = users;
}
...
Network::~Network() {
    ...
    users_.clear();  // Optional: clear the vector to remove dangling pointers
}
...
void Network::addUser(const std::string& name, int year, int zip) {
    User* user = new User(users_.size(), name, year, zip, std::set<int>());
    addUser(user);
}
